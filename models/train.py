...
class RotaryEmbedding(nn.Module):
    ...
    def rotate_embedding(self, x, cos, sin):
        ...
        result = mx.concatenate([x1 * cos - x2 * sin, x2 * cos + x1 * sin], axis=-1)

        return result


# Attention Mechanism with Precomputed Components
class LLaMAAttention(nn.Module):
    def __init__(self, dims: int, num_heads: int, max_seq_len: int):
        super().__init__()
        self.dims = dims
        ...
        # Precompute rotary embeddings and causal mask
        positions = mx.arange(max_seq_len)
        self.cos, self.sin = self.rope(positions)
        self.causal_mask = mx.tril(mx.ones((max_seq_len, max_seq_len)))
    ...
    def __call__(self, x):
        ...
        # Use precomputed rotary embeddings
        ...
        # Compute attention with precomputed causal mask
...
class LLaMABlock(nn.Module):
    def __init__(self, dims: int, num_heads: int, ffn_dims: int, max_seq_len: int):
        super().__init__()
        self.attention_norm = RMSNorm(dims)
        self.attention = LLaMAAttention(dims, num_heads, max_seq_len)
...
class LLaMAModel(nn.Module):
    def __init__(
        ...
        self.blocks = [
            LLaMABlock(dims, num_heads, ffn_dims, max_seq_len)
...
def save_checkpoint(model, optimizer, args, step, tokens_processed):
    ...
    try:
        ...
        mx.savez(
            os.path.join(checkpoint_dir, "optimizer.npz"), **dict(flat_optimizer_state)
        )
...
def main(args):
    ...
    if args.use_wandb:
        wandb.init(
            ...
            or f"llama-mlx-{args.dim}-{args.num_blocks}-{args.dataset_name}",
            config=vars(args),
        )
    ...
    logger.info(
        f"Initializing LLaMA model with {args.num_blocks} layers, "
    ...
    model = LLaMAModel(
        ...
        ffn_dims=ffn_dims,
        max_seq_len=args.context_size,  # Pass context_size as max_seq_len
    ...
    if args.resume_from:
        ...
        logger.info(
            f"Resuming from step {start_step} with {tokens_processed} tokens processed"
        )

    # Training loss function
    def loss_fn(model, x, y):
        logits = model(x)
        # Ensure proper shapes for cross entropy
    ...
    for inputs, targets in train_dataloader:
        ...
        if args.lr_schedule == "cosine" and args.num_iters > 0:
            ...
            optimizer.learning_rate = (
                min(1, step_count / args.lr_warmup) * args.learning_rate
            )
        ...
        try:
            ...
            if step_count % args.steps_per_report == 0:
                ...
                tokens_per_sec = (inputs.size * args.steps_per_report) / (
            ...
            if args.debug:
                ...
                logger.error(
                    f"Input dtype: {inputs.dtype}, Target dtype: {targets.dtype}"
                )
        ...
        if args.num_iters > 0 and step_count >= args.num_iters:
            logger.info(
                f"Reached max iterations ({args.num_iters}). Training complete."
            )
            break
    ...
    save_checkpoint(
        model, optimizer, args, step_count, train_dataloader.tokens_processed
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Train a LLaMA-style Model with MLX on FineWeb Dataset"
    )

    # Model architecture
    parser.add_argument("--dim", type=int, default=1024, help="Model dimension")
    parser.add_argument(
        "--num_blocks", type=int, default=16, help="Number of transformer layers"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--context_size", type=int, default=2048, help="Context window size"
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
        default=2_500_000_000,  # 2.5B tokens
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.1, help="AdamW weight decay"
    )
    parser.add_argument(
        "--lr_warmup", type=int, default=2000, help="Learning rate warmup steps"
    )
    parser.add_argument(
        "--lr_schedule",
        type=str,
    ...
    parser.add_argument("--gpu", action="store_true", help="Use Metal GPU backend")
    parser.add_argument(
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="llama-mlx", help="W&B project name"
    )
    parser.add_argument(
        "--wandb_run_name",
        type=str,
